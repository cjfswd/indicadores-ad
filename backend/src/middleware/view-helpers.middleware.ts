...
const TIPO_EVENTO_LABELS: Record<string, string> = {
  ...
  ea_broncoaspiracao: 'EA: Broncoaspiração', ea_lesao_pressao: 'EA: Lesão Pressão',
...
/** Audit log description builder — used by auditoria partial */
...
/** Check if audit entry can be reverted */
function canRevert(entry: { revertido: boolean; acao: string }): boolean {
  return !entry.revertido && ['criar', 'editar', 'confirmar', 'desativar', 'reativar', 'excluir'].includes(entry.acao)
}

/** Format audit payload for display */
function renderAuditPayload(entry: { payload: string | null; id: string; entidade: string; acao: string }): string {
  try {
...
/** Check if a month is within a seasonal range */
...
/** Register all view helpers as app.locals */
export function registerViewHelpers(app: Application): void {
  app.locals.fmtDate = fmtDate
  app.locals.fmtTs = fmtTs
  app.locals.calcularIdade = calcularIdade
  app.locals.MESES = MESES
  app.locals.MESES_CURTOS = MESES_CURTOS
  app.locals.INDICADOR_LABELS = INDICADOR_LABELS
  app.locals.TIPO_EVENTO_LABELS = TIPO_EVENTO_LABELS
  app.locals.SENTIDO_CONFIG = SENTIDO_CONFIG
  app.locals.ACAO_COLORS = ACAO_COLORS
  app.locals.ENTIDADE_LABELS = ENTIDADE_LABELS
  app.locals.ICON_COLORS = ICON_COLORS
  app.locals.getAuditDescription = getAuditDescription
  app.locals.canRevert = canRevert
  app.locals.renderAuditPayload = renderAuditPayload
  app.locals.isMonthActive = isMonthActive
}
