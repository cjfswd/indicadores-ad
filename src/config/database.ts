...
const useExternalPostgres = DATABASE_URL.startsWith('postgresql://') || DATABASE_URL.startsWith('postgres://')

/**
 * PostgreSQL DDL shared between PGlite (dev) and external PostgreSQL (prod).
 * Single source of truth — no more dual-dialect divergence.
 */
const PG_SCHEMA_DDL = `
...
export async function initializeDatabase() {
  if (useExternalPostgres) {
    ...
    logger.info('Database PostgreSQL (externo) conectado')
  } else {
    // ── Development: PGlite — PostgreSQL WASM in-memory ──
    const { PGlite } = await import('@electric-sql/pglite')
    const { PGliteDialect } = await import('kysely-pglite-dialect')

    const pglite = new PGlite()

    // pglite.exec() supports multi-statement SQL (unlike prepared statements)
    await pglite.exec(PG_SCHEMA_DDL)

    db = new Kysely<Database>({
      dialect: new PGliteDialect(pglite),
    })

    logger.info('Database PGlite in-memory (PostgreSQL WASM) inicializado')
  }
}
...
/** Whether running external PostgreSQL (production) */
export function isProduction(): boolean {
  return useExternalPostgres
}
